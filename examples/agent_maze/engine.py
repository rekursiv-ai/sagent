...
PRESS_WINDOW = 8  # house-ignore[globals] -- Game-balance tuning dial.
...
class Engine:
    ...
    def _build_scene(self, model: str) -> dict[str, Any]:
        ...
        for xy, li in w._plate_lock.items():  # noqa: SLF001 -- The example engine accesses the world’s private topology to enforce its puzzle protocol.
    ...
    def move(self, aid: str, x: int, y: int) -> str:
        ...
        while a.xy != (x, y) and steps < 300:
            nxt = self.world._bfs_next(a.xy, (x, y))  # noqa: SLF001 -- The example engine accesses the world’s private topology to enforce its puzzle protocol.
    ...
    def press(self, aid: str, partner: str) -> str:
        ...
        self.t += 1
        a = self.world.agents[aid]
        if a.xy not in self.world._plate_lock:  # noqa: SLF001 -- The example engine accesses the world’s private topology to enforce its puzzle protocol.
        ...
        li = self.world._plate_lock[a.xy]  # noqa: SLF001 -- The example engine accesses the world’s private topology to enforce its puzzle protocol.
        self.emit(
            aid,
            "press",
            outcome="armed",
...
def _local_map(world: World, aid: str) -> str:
    ...
    for dy in range(-s, s + 1):
        ...
        for dx in range(-s, s + 1):
            ...
            if (dx, dy) == (0, 0):
                row += "@"
            elif (x, y) in world._plate_lock:  # noqa: SLF001 -- The example engine accesses the world’s private topology to enforce its puzzle protocol.
                row += PLATE_LETTERS[world._plate_lock[(x, y)]]  # noqa: SLF001 -- The example engine accesses the world’s private topology to enforce its puzzle protocol.
            elif (x, y) in occ:
                row += "o"
