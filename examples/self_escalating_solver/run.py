...
"""
    with tempfile.NamedTemporaryFile(
        ...
        path = fh.name
    try:
        r = subprocess.run(  # noqa: S603 -- Argv is a fixed list built here, never from user input.
            ["uv", "run", "--with", "numpy", "--with", "scipy", "python", path],  # noqa: S607 -- The executable is fixed by the example, never user input.
            capture_output=True,
            text=True,
            check=False,
            timeout=180,
