...
# --------------------------------------------------------------------------
_ORACLE_TOKEN: Final = "ZK9Q7M"  # noqa: S105 -- This is a placeholder credential used only by the example fixture.
_ORACLE_SRC = f"""
import numpy as _np
...
def _parse_verdict(out: str) -> str | None:
    hits = re.findall(rf"ORACLE_{_ORACLE_TOKEN}:\s*(PASS|FAIL[^\n]*)", out or "")
    if not hits:
        return None
...
class RunPython:
    ...
    def _exec(self, code: str) -> str:
        ...
        with tempfile.NamedTemporaryFile(
            ...
            path = fh.name
        try:
            r = subprocess.run(  # noqa: S603 -- The example launches a fixed solver command assembled locally.
                ["uv", "run", "--with", "numpy", "--with", "scipy", "python", path],  # noqa: S607 -- The example executable is selected from its fixed solver configuration.
                capture_output=True,
                text=True,
                check=False,
                timeout=120,
...
async def run_condition(
    ...
    *,
    ...
    try:
        ...
        )  # ty: ignore[redundant-cast] -- The cast documents the runtime protocol across an untyped solver boundary.  # pyright: ignore[reportUnnecessaryCast] -- Agent's import boundary is unknown to basedpyright.
        async for _ in events:
            pass
    except Exception as e:  # noqa: BLE001 -- The example boundary reports any solver failure without aborting cleanup.
        err = f"{type(e).__name__}: {str(e)[:100]}"

    history = cast(list[ModelContextEvent], agent.history)  # ty: ignore[redundant-cast] -- The cast documents the runtime protocol across an untyped solver boundary.  # pyright: ignore[reportUnnecessaryCast] -- Agent's import boundary is unknown to basedpyright.
