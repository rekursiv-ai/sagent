...
class TestConditionalMatching:
    ...
    def test_matches_py_file(
        ...
        _ = (tmp_path / "foo.py").write_text("x = 1\n")
    ...
    def test_no_match_for_non_py(
        ...
        self._setup_python_rule(tmp_path)
        _ = (tmp_path / "README.md").write_text("text\n")
        matches = agents_md._matching_file_triggered_md(
            tmp_path,
            [tmp_path / "README.md"],
            config=cfg,
    ...
    def test_unconditional_rule_not_returned(
        ...
        _ = (rules_dir / "always.md").write_text("unconditional\n")
        matches = agents_md._matching_file_triggered_md(
            tmp_path,
            [tmp_path / "foo.py"],
            config=cfg,
...
class TestRuleMatching:
    def test_no_globs_does_not_match(self, tmp_path: Path) -> None:
        rule = agents_md._AgentMdFile(
            path=tmp_path / "AGENTS.md",
            content="x",
            memory_type="Project",
        )
        assert not agents_md._rule_matches(
            rule, tmp_path / "foo.py", tmp_path, ".sagent"
        )

    def test_target_outside_base_returns_false(self, tmp_path: Path) -> None:
        rule = agents_md._AgentMdFile(
            path=tmp_path / ".sagent" / "rules" / "python.md",
            content="x",
            memory_type="Project",
            globs=["**/*.py"],
        )
        outside = tmp_path.parent / "elsewhere.py"
        assert agents_md._rule_matches(rule, outside, tmp_path, ".sagent") is False

    def test_managed_uses_cwd_base(self, tmp_path: Path) -> None:
        rule = agents_md._AgentMdFile(
            path=Path("/etc/sagent/rules/managed.md"),
            content="m",
            memory_type="Managed",
            globs=["**/*.py"],
        )
        ...
        _ = target.write_text("")
        base = agents_md._rule_base_dir(rule, tmp_path, ".sagent")
        assert base == tmp_path

    def test_rule_base_dir_falls_back_to_parent(self, tmp_path: Path) -> None:
        rule = agents_md._AgentMdFile(
            path=tmp_path / "stray" / "AGENTS.md",
            content="x",
            memory_type="Project",
            globs=["**/*.py"],
        )
        assert agents_md._rule_base_dir(rule, tmp_path, ".sagent") == tmp_path / "stray"


class TestMatchingDedup:
    def test_same_rule_path_not_doublecounted(
        self, tmp_path: Path, cfg: agents_md.AgentsMdConfig
    ) -> None:
        rules = tmp_path / ".sagent" / "rules"
        rules.mkdir(parents=True)
        _ = (rules / "py.md").write_text(
            "---\npaths:\n  - 'a/*.py'\n  - 'b/*.py'\n---\nrule\n"
        )
        matches = agents_md._matching_file_triggered_md(
            tmp_path,
            [tmp_path / "a" / "x.py", tmp_path / "b" / "y.py"],
            config=cfg,
        )
        assert len(matches) == 1
