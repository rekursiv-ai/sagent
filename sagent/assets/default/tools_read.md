...
- `file_path` absolute; relative paths resolve against the session cwd. Default: the whole file, line-numbered (right-aligned number + tab + content, like `cat -n`).
...
- Empty files return `[File exists but is empty: <path>]`, not blank content.
