...
def test_parse_cli_args_session_paths() -> None:
    ns = _parse(["--session", "/tmp/x"])  # noqa: S108 -- Path is a fixture string, never opened.
    assert ns.session == "/tmp/x"  # noqa: S108 -- Path is a fixture string, never opened.
...
def test_resolve_session_dir_explicit_session() -> None:
    ns = _parse(["--session", "/tmp/explicit"])  # noqa: S108 -- Path is a fixture string, never opened.
    assert _resolve_session_dir(ns) == "/tmp/explicit"  # noqa: S108 -- Path is a fixture string, never opened.
...
def test_direct_script_bootstraps_dependencies() -> None:
    ...
    script = _CWD / "cli.py"
    proc = subprocess.run(  # noqa: S603 -- Argv is a fixed list built here, never from user input.
        [str(script), "--help"],
        capture_output=True,
        text=True,
        timeout=120,
