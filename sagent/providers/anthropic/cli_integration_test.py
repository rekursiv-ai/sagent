...
def _claude_available() -> bool:
    ...
    if status is not None:
        return status
    creds = credentials_path(Path.home() / ".claude" / ".credentials.json", None)  # noqa: TID251 -- The integration test imports the vendor CLI compatibility layer.
    return creds.exists()
...
def _probe_claude_auth() -> str:
    ...
    try:
        proc = subprocess.run(
            [  # noqa: S607 -- The integration test invokes the fixed Claude CLI executable.
                "claude",
                "--print",
                "--output-format",
                "json",
