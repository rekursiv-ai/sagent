        ...
        for token in self._flatten_tokens(self.parsed):
            ...
            if node_type == "text":
                ...
                if exiting:
                    ...
                    context.stack.pop()
                    text = token.content
                    if text is not None:  # pyright: ignore[reportUnnecessaryComparison] -- The vendored Rich token contract may be optional at runtime.  # ty: ignore[redundant-condition-strict] -- The vendored Rich token contract may be optional at runtime.
