...
_LEGACY_SAGENT_HOME = Path.home() / ".sagent"  # noqa: TID251 -- vendor fixed path, not ours (AGENTS.md rule 3)  # house-ignore[xdg-literal, module-side-effects] -- The pre-XDG sagent home being migrated FROM, not our layout; a module constant so tests can repoint the migration source.
_LEGACY_CLAUDE_HOME = Path.home() / ".claude"  # noqa: TID251 -- vendor fixed path, not ours (AGENTS.md rule 3)  # house-ignore[xdg-literal, module-side-effects] -- Vendor CLI's fixed home path, not ours (AGENTS.md rule 3).
