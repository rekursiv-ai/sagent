...
def test_cwd_slug_no_separator_collision() -> None:
    ...
    """
    assert cwd_slug("/tmp/a-b") != cwd_slug("/tmp/a/b")  # noqa: S108 -- The path is a test fixture string and is never opened.
...
def test_cwd_slug_escape_is_unambiguous() -> None:
    ...
    """
    assert cwd_slug("/tmp/x-2e-y") != cwd_slug("/tmp/x.y")  # noqa: S108 -- The path is a test fixture string and is never opened.
    assert cwd_slug("/tmp/a-5f-b") != cwd_slug("/tmp/a_b")  # noqa: S108 -- The path is a test fixture string and is never opened.
...
def test_cwd_slug_no_underscore_collision() -> None:
    ...
    """
    assert cwd_slug("/tmp/a_b") != cwd_slug("/tmp/a-b")  # noqa: S108 -- The path is a test fixture string and is never opened.
    assert cwd_slug("/tmp/a.b") != cwd_slug("/tmp/a-b")  # noqa: S108 -- The path is a test fixture string and is never opened.
...
def _info(idx: int) -> SessionInfo:
    ...
    return SessionInfo(
        path=Path(f"/tmp/s{idx}"),  # noqa: S108 -- The path is a test fixture string and is never opened.
