...
def set_recipe(name_or_path: str) -> None:
    ...
    """
    global _recipe_path_override, _recipe_cache  # noqa: PLW0603 -- Decorator registration must update the process-wide recipe override and cache.
...
_MAX_ASSET_DEPTH = 8  # house-ignore[globals] -- Include-recursion depth dial.
...
def _load_recipe() -> dict[str, object]:
    ...
    global _recipe_cache  # noqa: PLW0603 -- The recipe cache is shared by all tools in the agent process.
...
def _read_asset(path: str | Path, *, visited: set[str], depth: int) -> str:
    ...
    try:
        return re.sub(r"\{\{include:\s*(.+?)\}\}", _replace, text)
    finally:
