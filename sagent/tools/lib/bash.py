    ...
    *,
    ...
    for i in reversed(range(len(stages))):
        if stages[i].kind != "command":
            ...
            # nudged although ``sort`` transforms the output. No tool claims
            # the NUL-prefixed name and no sink rule treats it as shaping, so
            # it correctly blocks the stages it separates.
            linked[i] = Invocation(
                exe="\0compound",
                args=(),
                cwd=cwd,
