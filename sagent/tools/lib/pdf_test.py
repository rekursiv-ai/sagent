...
def test_extracted_jpeg_is_decodable(tmp_path: Path) -> None:
    ...
    img.load()  # pyright: ignore[reportUnknownMemberType] -- Pillow's deferred-import module leaves Image.load's native return type unavailable to pyright.
...
def test_concurrent_extract_does_not_corrupt_heap(tmp_path: Path) -> None:
    ...
    def work(p: Path) -> None:
        ...
        except BaseException as e:  # noqa: BLE001 -- The thread harness must capture native rasterizer faults so the parent assertion can report them.
            errors.append(e)
