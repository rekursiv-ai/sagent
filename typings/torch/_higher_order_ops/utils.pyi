...
def potential_input_alias_or_mutation(
    ...
    tuple[tuple[dict[int, int], dict[int, int], dict[int, int]], list[int]]
    | Literal[True]
): ...
def analyze_potential_input_alias_or_mutation(
    name, aliases, input_mutations
) -> None: ...
...
def prepare_fw_with_masks_all_requires_grad(
    fn,
) -> Callable[..., tuple[PyTree | Any, PyTree]]: ...
...
def create_fw_bw_graph(
    fn, use_output_and_grad_bw, fw_inputs, fw_outputs
...
def mask_list(
    mask: list[bool], inp: list[Any], other: list[Any] | None = ...
) -> list[Any]: ...
def first_slice_copy_with_grad(li: Iterable[Any]) -> list[Any]: ...
def diff_tensor_meta(
    meta1: TensorMetadata, meta2: TensorMetadata, check_grad=...
) -> list[str]: ...
...
def filter_with_masks(
    data: list[torch.Tensor | None], masks: list[bool]
) -> list[Tensor | None]: ...
def fill_none_with_masks(
    data: list[torch.Tensor | None], masks: list[bool]
) -> list[Tensor | None]: ...
