...
def check_all_strides(
    a: TensorLikeType, b: TensorLikeType, *, only_cuda=...
) -> tuple[bool, int | None]: ...
...
def is_contiguous_for_memory_format(
    a: Tensor, *, memory_format: torch.memory_format, false_if_dde=...
) -> bool: ...
...
def canonicalize_dims(
    rank: int, indices: Sequence[int], wrap_scalar: bool = ...
) -> tuple[int, ...]: ...
@overload
...
def extract_shape_from_varargs(
    shape: ShapeType | tuple[ShapeType], validate=...
) -> tuple[int, ...]: ...
...
def check_fp_or_complex(
    dtype: torch.dtype, fn_name: str, allow_low_precision_dtypes: bool = ...
) -> None: ...
...
def make_contiguous_strides_for(
    shape: ShapeType, row_major: bool = ...
) -> tuple[_IntLikeT | int, ...]: ...
...
def set_correction(
    unbiased: bool | None = ..., correction: NumberType | None = ...
) -> float: ...
...
@deprecated(
    "`torch._prims_common.check` is deprecated and will be removed in the future. Please use `torch._check*` functions instead.",
...
class CUDARngStateHelper:
    ...
    def get_torch_state_as_tuple(
        fake_mode: AbstractContextManager[Any] = ...,
    ) -> tuple[Tensor, Tensor]: ...
    @staticmethod
